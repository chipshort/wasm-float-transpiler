//! Taken from wasmer-wast and modified to replace float constants
...
use std::str;
use wasmer::*;
use wasmer_wast::{spectest_importobject, DirectiveError, DirectiveErrors};
use wast::{NanPattern, WastExecute, WastInvoke};
...
impl Wast {
    ...
    fn runtime_value(&mut self, v: &wast::Expression<'_>) -> Result<Val> {
        ...
        Ok(match &v.instrs[0] {
            ...
            F32Const(x) => Val::I32(x.bits as i32),
            F64Const(x) => Val::I64(x.bits as i64),
    ...
    fn val_matches(&self, actual: &Val, expected: &wast::AssertExpression) -> Result<bool> {
        Ok(match (actual, expected) {
            ...
            (Val::I32(a), wast::AssertExpression::F32(b)) => f32_matches(*a as u32, b),
            (Val::I64(a), wast::AssertExpression::F64(b)) => f64_matches(*a as u64, b),
...
fn f32_matches(actual: u32, expected: &wast::NanPattern<wast::Float32>) -> bool {
    match expected {
        wast::NanPattern::CanonicalNan => simple_soft_float::F32::from_bits(actual).is_quiet_nan(),
        wast::NanPattern::ArithmeticNan => simple_soft_float::F32::from_bits(actual).is_nan(),
...
fn f64_matches(actual: u64, expected: &wast::NanPattern<wast::Float64>) -> bool {
    match expected {
        wast::NanPattern::CanonicalNan => simple_soft_float::F64::from_bits(actual).is_quiet_nan(),
        wast::NanPattern::ArithmeticNan => simple_soft_float::F64::from_bits(actual).is_nan(),
        wast::NanPattern::Value(expected_value) => actual == expected_value.bits,
    }
}
